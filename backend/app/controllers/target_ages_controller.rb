class TargetAgesController < ApplicationController
  ...
  def create
    ...
    return render json: { message: '成功しました' }, status: 200 if target_ages.save
  ...
  def update
    ...
    return render json: { message: '成功しました' }, status: 200 if target_age.update(create_params)
  ...
  def destroy
    target_age = TargetAge.find(params[:id])
    return render json: { message: '削除に成功しました' }, status: 200 if target_age.destroy
